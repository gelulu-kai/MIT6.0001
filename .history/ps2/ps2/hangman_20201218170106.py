...
vowels = ['a','e','i','o']  #store the vowels
    ...
    * After each guess, you should display to the user the 
    ...
    while guess_chance > 0:
      ...
      if my_guess not in all_lowercase or len(my_guess) != 1:
        ...
        if warning_chance <0:
          guess_chance-=1
          print('Oops! That is not a vaild letter. You have no warnings left!')
          print('So you lose one guess: %s'%(get_guessed_word(secret_word,guessed)))
          print('------------')
          continue
        else:
          print('Oops! That is not a vaild letter. You have %d warnings left:%s'%(warning_chance,get_guessed_word(secret_word,guessed)))
          print('------------')
          continue

      if my_guess not in available_letters:  #已经猜过
        warning_chance-=1
        if warning_chance<0:
          guess_chance-=1
          ...
          print("Oops! You've already guessed that letter.You have %d warnings left: %s"%(warning_chance,get_guessed_word(secret_word,guessed)))
          print("------------")
          continue
      else:
        if my_guess not in secret_word:  #未猜过且未猜中
          if guess_chance == 1:
            guess_chance = 0
    ...
    if sucess==True:
      ...
      print("You total  score for this game is: ",(guess_chance*unique_letters(secret_word)))
    else:
