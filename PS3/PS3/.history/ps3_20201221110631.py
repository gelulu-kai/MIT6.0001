...
def get_word_score(word, n):
    ...
    score = []
    for each in word:
        score.append(SCRABBLE_LETTER_VALUES.get(each))
    letter_points = sum(int(score))
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' not in word:
        for i in range(len(word)):
            if word[i] in hand and hand[word[i]] >= word.count(word_copy[i]) and word in word_list:
                test.append(1)
            else:
                test.append(0)
...
def substitute_hand(hand, letter):
    ...
    if letter in hand:
        ...
        num_vow = math.ceil(num_letter/3)  #向上取整
        del new_hand[letter]
        while sum(new_hand.values())<sum(hand.values()):
            ...
            if sum(new_hand.values())>= sum(hand.values()):
                break
            for i in range(num_vow,num_letter):
                x = random.choice(CONSONANTS)
                if x not in hand:
                    new_hand[x]= new_hand.get(x,0)+1
        return new_hand
